...
@interface PJSIPViewController ()<ZDSipManagerDelegate>{
    NSTimer * _timer;
    NSInteger _timeTick;
...
@implementation PJSIPViewController
...
- (void)viewDidLoad {
    ...
    [self initUI];
    [[PJSipManager manager]dailWithPhonenumber:self.model.phone];
...
-(void)initUI{
    self.callCenterPhoneStype = CallingPhoneStype;
    [self.view addSubview:self.callCenterView];
    [self.callCenterView.time_lbl setText:@"请耐心等待"];
}
...
-(CRMCallCenterView *)callCenterView{
    if (!_callCenterView) {
        ...
        [_callCenterView relaodWithTelPhoneStype:self.callCenterPhoneStype model:self.model phoneStutas:@"正在呼叫..."];
...
-(void)callBuutonEvent:(CustomBtn *)sender{
    ...
    if (tag == 1000) {//静音
        ...
        [[PJSipManager manager] callingHangup];
        [self delay:1.0 task:^{
            [self viewDissmiss];
        }];
    }
...
- (void)handleCallStatusChanged:(NSNotification *)notification {
    ...
    NSLog(@"电话状态==========================: %d",state);
    NSLog(@"电话状态INfo==========================: %@",notification.userInfo);
    if (state == PJSIP_INV_STATE_CONNECTING) {
        ...
        [[AVSound sharedInstance] stop];
        HHLog(@"==通话已建立连接==");
        [_callCenterView.mute_btn setHidden:NO];
        [_callCenterView.handsfree_btn setHidden:NO];
